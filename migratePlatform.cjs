...
function processFile(filePath) {
  ...
  const tagline = taglineMatch ? taglineMatch[1].trim() : 'PLATFORM';

  // Extract title
  const titleMatch = content.match(/<h1[^>]*>\s*([\s\S]*?)\s*<\/h1>/) || content.match(/<motion\.h1[^>]*>\s*([\s\S]*?)\s*<\/motion\.h1>/);
  ...
  const subtitle = subtitleMatch ? subtitleMatch[1].trim().replace(/\s+/g, ' ').replace(/—/g, '-').replace(/<br\s*\/?>/gi, '') : 'Platform Capability';

  // Extract description
  const descMatch = content.match(/<p[^>]*text-muted-foreground[^>]*>\s*([\s\S]*?)<\/p>/) || content.match(/<motion\.p[^>]*text-zinc-300[^>]*>\s*([\s\S]*?)<\/motion\.p>/);
  const description = descMatch ? descMatch[1].trim().replace(/\s+/g, ' ').replace(/—/g, '-').replace(/<br\s*\/?>/gi, '') : '';

  // Identify features array name
  const featuresNameMatch = content.match(/const ([A-Za-z0-9_]+) = \[\s*\{\s*(title|num|icon|step|feature|name)/);
  const featuresName = featuresNameMatch ? featuresNameMatch[1] : '[]';

  // Inject the hook and conditional return
  const hookInject = `\n  const { designSystem } = useThemeCustomizer();\n\n  if (designSystem === "ebay") {\n    return (\n      <EbaySolutionTemplate \n        tagline="${tagline}" \n        title="${title}" \n        subtitle="${subtitle}" \n        description="${description}" \n        features={${featuresName}.map((f:any)=>({title: f.title || f.tag || f.feature || f.name || '', desc: f.desc || f.headline || f.body || f.description || ''}))}\n      />\n    );\n  }\n`;
...
const folders = fs.readdirSync(pagesDir).filter(f => f.startsWith('platform-') || f.startsWith('about-us'));
folders.forEach(f => {
