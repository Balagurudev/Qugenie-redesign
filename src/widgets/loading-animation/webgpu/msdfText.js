...
export default class MSDFText {
  ...
  async initialize(
    ...
    const heightOffset = (textGeometry.layout.height / 2) * textScale;

    mesh.position.set(position.x + meshOffset, position.y - heightOffset, position.z);
  ...
  createTextMaterial(fontAtlasTexture, perlinTexture, uProgress) {
    ...
    const uDissolvedColor = uniform(new THREE.Color('#0040C1'));
    const uDesatComplete = uniform(0.45);
    const uBaseColor = uniform(new THREE.Color('#ffffff'));
