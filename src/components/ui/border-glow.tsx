...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  useEffect(() => {
    ...
    if (loop) {
      setSweepActive(true);
      setEdgeProximity(1); // Keep border fully visible always
      ...
      const tick = () => {
        ...
        // Make it fast and continuous: full rotation every 1500ms
        const degrees = (elapsed / 1500) * 360; 
        setCursorAngle(degrees % 360);
    ...
    const runAnimation = () => {
      ...
      animateValue({ duration: 500, onUpdate: v => { if (isMounted) setEdgeProximity(v / 100) } });
      animateValue({ ease: easeInCubic, duration: 1500, end: 50, onUpdate: v => {
        if (isMounted) setCursorAngle((angleEnd - angleStart) * (v / 100) + angleStart);
      }});
      animateValue({ ease: easeOutCubic, delay: 1500, duration: 2250, start: 50, end: 100, onUpdate: v => {
        if (isMounted) setCursorAngle((angleEnd - angleStart) * (v / 100) + angleStart);
      }});
      animateValue({ ease: easeInCubic, delay: 2500, duration: 1500, start: 100, end: 0,
        onUpdate: v => { if (isMounted) setEdgeProximity(v / 100) },
        onEnd: () => {
          if (isMounted) setSweepActive(false);
        },
      });
    };
